...
async function updateQuoteDynamoDBObject() {
  ...
  try {
    var quoteParams = {
      ...
      UpdateExpression: "SET #qg = #qg + :inc",
  ...
  async function getRandomQuote(apiURLInput) {
    ...
    const svgImage = `
            <svg width="${width}" height="${height}">
              ...
              <text x="${width / 2}" y="${
      height - 10
    }" class="footerStyles">Developed by @GiulianoSoria | Quotes from ZenQuotes.io</text>
            </svg>
          `;
    console.log(svgImage);
    // Add background images for the svg creation
    const backgroundImages = [
      "backgrounds/Amethyst.png",
      "backgrounds/Hydrogen.png",
      "backgrounds/Sylvia.png",
